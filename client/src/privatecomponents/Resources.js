...
const Resources = () => {
      ...
      if(resourcename && rdescription && appurl){
        for(let i=0;i<selectedFiles.length;i++){
          ...
          axios.post('https://api.cloudinary.com/v1_1/drnndbow7/image/upload',formData).then(res =>{
            ...
            axios.post('http://localhost:5000/addresource',{Rname:resourcename,Resourcedescription:rdescription,weburl:appurl,pic:res.data.url},{headers : {'x-token' : localStorage.getItem('token')}}).then(res =>{ console.log(res.data); setAllresources(res.data.update)  });
  ...
  useEffect(()=>{
    axios.get('http://localhost:5000/getresource',{headers : {'x-token' : localStorage.getItem('token')}}).then(
  ...
  return ( 
    ...
    <div >
      <Header3 />   
      <center>
        <h2 style={{alignItems:"center"}}><b>-: Students Resources :-</b></h2><br /><br />

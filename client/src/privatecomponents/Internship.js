...
const Internship = () => {
   ...
   const {compName,compWebsite,description,requirements,mobile,email,expDate,stipend,applyPage,role} = data
    ...
    const internshipsubmithandler = e =>{ 
        ...
        {
            axios.post('http://localhost:5000/addinternship',data,{headers : {'x-token' : localStorage.getItem('token')}}).then(
    ...
    useEffect(()=>{
        axios.get('http://localhost:5000/getinternship',{headers : {'x-token' : localStorage.getItem('token')}}).then(
        res => 

...
const Mentors = () => {
  ...
  const {mentorname,clgId,dept,mobile,email,domains,expert} = data
   ...
   const mentorsubmithandler = e =>{ 
        ...
        {
            axios.post('http://localhost:5000/addmentor',data,{headers : {'x-token' : localStorage.getItem('token')}}).then(
  ...
  useEffect(()=>{
    axios.get('http://localhost:5000/getmentors',{headers : {'x-token' : localStorage.getItem('token')}}).then(
  ...
  return ( 
    <div >
      ...
      <center>
        ...
        <h2 style={{alignItems:"center"}}><b>-: Students Mentors :-</b></h2><br /><br />

...
const Aluminiprofile = () => {
    ...
    var x="http://localhost:5000/indprofile/"
    var y="http://localhost:5000/indprofile2/"
    ...
    const pgHandler = e  =>{
        e.preventDefault();
        if(pg){
            axios.put(`http://localhost:5000/addpgA/${iprofile._id}`,{ pg:pg },{headers : {'x-token' : localStorage.getItem('token')}}).then(
    ...
    const expHandler = e  =>{
        e.preventDefault();
        if(experience){
            axios.put(`http://localhost:5000/addexpA/${iprofile._id}`,{ experience:experience },{headers : {'x-token' : localStorage.getItem('token')}}).then(
                response => {alert(response.data.message); setIprofile(response.data.update)}
            );
            setExperience("")
        }
    ...
    const projectHandler = e  =>{
        e.preventDefault();
        if(project){
            axios.put(`http://localhost:5000/addprojectA/${iprofile._id}`,{ project:project },{headers : {'x-token' : localStorage.getItem('token')}}).then(
                response => {alert(response.data.message); setIprofile(response.data.update)}
            );
            setProject("")
        }
    ...
    const bioHandler = e  =>{
        e.preventDefault();
        if(bio){
            axios.put(`http://localhost:5000/addbioA/${iprofile._id}`,{ bio:bio },{headers : {'x-token' : localStorage.getItem('token')}}).then(
                response => {alert(response.data.message); setIprofile(response.data.update)}
            );
            setBio("")
        }
    ...
    const quoteHandler = e  =>{
        e.preventDefault();
        if(quote){
        axios.put(`http://localhost:5000/addquoteA/${iprofile._id}`,{ quote:quote },{headers : {'x-token' : localStorage.getItem('token')}}).then(
            response => {alert(response.data.message); setIprofile(response.data.update)}
        );
        setQuote("")
        }
    ...
    const skillHandler = e  =>{
        e.preventDefault();
        if(skill){
        axios.put(`http://localhost:5000/addskillA/${iprofile._id}`,{ skill:skill },{headers : {'x-token' : localStorage.getItem('token')}}).then(
            response => {alert(response.data.message); setIprofile(response.data.update)}
        );
        setSkill("")
        }
    ...
    const jobHandler = e  =>{
        ...
        {
            axios.put(`http://localhost:5000/addjobA/${iprofile._id}`,jobdata,{headers : {'x-token' : localStorage.getItem('token')}}).then(
                response => {alert(response.data.message); setIprofile(response.data.update)}
            );
    ...
    const mobileHandler = e  =>{
        e.preventDefault();
        if(mobile.length===10){
        axios.put(`http://localhost:5000/addmobileA/${iprofile._id}`,{ mobile:mobile },{headers : {'x-token' : localStorage.getItem('token')}}).then(
            response => {alert(response.data.message); setIprofile(response.data.update)}
        );
        setMobile("")
        }
    ...
    const githubHandler = e  =>{
        e.preventDefault();
        if(github.length>=20){
        axios.put(`http://localhost:5000/addgithubA/${iprofile._id}`,{ github:github },{headers : {'x-token' : localStorage.getItem('token')}}).then(
            response => {alert(response.data.message); setIprofile(response.data.update)}
        );
        setGithub("")
        }
    ...
    const linkedinHandler = e  =>{
        ...
        axios.put(`http://localhost:5000/addlinkedinA/${iprofile._id}`,{ linkedin:linkedin },{headers : {'x-token' : localStorage.getItem('token')}}).then(
            response => {alert(response.data.message); setIprofile(response.data.update)}
        );
        setLinkedin("")
        }
    ...
    const deletereviewhandler = id =>{
        axios.delete(`http://localhost:5000/deletereview/${id}`,{headers : {'x-token' : localStorage.getItem('token')}}).then(
        ...
        <div>
            ...
            {/* ------  mobile -----  */}
                        ...
                        <div>
                            ...
                            { h === 1 ? 
                                <div>
                                    <form onSubmit={mobileHandler} autoComplete="off">
            ...
            {/* ------  post gradutation -----  */}
                        {iprofile.pg ? 
                            <div>
                            <b>Post Graduation : </b>{iprofile.pg} <br />
                            <div>
            ...
            {/* ------  github info -----  */}
                        ...
                        {iprofile.github ? 
                            ...
                            <div><b>Github : </b><a href={iprofile.github} rel="noreferrer" target="_blank">{iprofile.github}</a>
                            <br />
                            <div>
            ...
            {/* ------  inkedin info -----  */}
                        ...
                        {iprofile.linkedin ? 
                            <div><b>Linkedin : </b><a href={iprofile.linkedin} rel="noreferrer" target="_blank">{iprofile.linkedin}</a><br />
                            <div>
                                        ...
                                        :
                                            <div>
                                                ...
                                                { b === 1 ? 
                                                    <div>
                                                        <form onSubmit={expHandler}>
                                                            <textarea className='textareaa' placeholder="Enter your experience info" value={experience} onChange={(e)=> setExperience(e.target.value)}  rows="3" cols="50"></textarea><br />
                                                            <input type="submit" className="btn btn-primary" value="submit" />
                                                        </form>
                                                        <br />
                                                    </div>
                                                ...
                                                { c === 1 ? 
                                                    <div>
                                                        <form onSubmit={projectHandler}>
                                                            <textarea className='textareaa' placeholder="Enter your projects info" value={project} onChange={(e)=> setProject(e.target.value)}  rows="3" cols="50"></textarea><br />
                                                            <input type="submit" className="btn btn-primary" value="submit" />
                                                        </form>
                                                        <br />
                                                    </div>
                                            ...
                                            { d === 1 ? 
                                                <div>
                                                    <form onSubmit={bioHandler}>
                                                        <textarea className='textareaa' placeholder="Enter your bio" value={bio} onChange={(e)=> setBio(e.target.value)}  rows="3" cols="50"></textarea><br />
                                                        <input type="submit" className="btn btn-primary" value="submit" />
                                                    </form><br />
                                                </div>
                                            :
                                                ...
                                                { e === 1 ? 
                                                    <div>
                                                        <form onSubmit={quoteHandler}>
                                                            <textarea className='textareaa' placeholder="Enter your best Quote" value={quote} onChange={(e)=> setQuote(e.target.value)}  rows="3" cols="50"></textarea><br />
                                                            <input type="submit" className="btn btn-primary" value="submit" />
                                                        </form><br />
                                                    </div>
                                                :
